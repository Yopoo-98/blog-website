 ...
 $conn = mysqli_connect("localhost","root","","blog");
  ...
  if($_FILES["image"]["error"]=== 4){
    ...
    if(!in_array($imageExtension,$validImageExtension)){
        ...
        $sql = "INSERT INTO `admin_profile_photo`(`admin_id`,`email`, `profession`, `image`,`date`) VALUES ('$admin_id','$email','$profession','$newImageName',now() )";

        $query = mysqli_query($conn, $sql);

        if($query){
            $_SESSION['message'] = "Admin Photo Added Successfully" . mysqli_error($conn);
            header("Location: super_profile.php?msg=Admin Profile Photo Added Successfully. ");
            exit(0);
        }
        else{
            $_SESSION['message'] = "Admin Profile Photo Not Updated Successfully" . mysqli_error($conn);
            header("Location: profile.php?msg=Admin Profile Photo Not Updated Successfully. ");
            exit(0);
        }
            ...
            <table id="datatableid" class="table"  style="border:2px solid black; background-color:gray;" >
              <thead style="font-size:13px;" >
                <tr>
                  ...
                  <th class="text-dark" style="border: 1px solid black;background:white">Profession</th>
                  <th class="text-dark" style="border: 1px solid black;background:white">Email</th>
                  <th class="text-dark" style="border: 1px solid black;background:white">Profile Image</th>
                      ...
                      ?>
                        <tr>
                          ...
                          <td style="border: 1px solid black;background:white;">
                            <a href="super_update_admin_profile.php?update_id=<?php echo $row['admin_profile_id']?>" id="post-update">Update</a>
                            <a onclick="return confirm('Do You Really Want to Delete This Record? ')"  href="super_delete_admin_profile.php?delete_id=<?php echo $row['admin_profile_id']?>" id="post-delete">Delete</a> 
                          </td>
