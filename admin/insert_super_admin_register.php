...
if (isset($_POST['sign_up'])) {
    ...
    if (mysqli_num_rows($run_email) > 0) {
        header("Location: add_new_user.php?msg=Try another email, this email already exists!");
        exit();
    }

    // Check if passwords match
    if ($password !== $confirm_password) {
        header("Location: add_new_user.php?msg=Passwords do not match!");
        exit();
    }

    // Check password length (6-9 characters)
    if (strlen($password) < 6 || strlen($password) > 9) {
        header("Location: add_new_user.php?msg=Password must be between 6 and 9 characters!");
        exit();
    }
    ...
    if ($query) {
        header("Location: admin_manage_users.php?msg=New user registered successfully!");
        exit();
    } else {
        header("Location: add_new_user.php?msg=Registration failed. Please try again.");
        exit();
    }
}
?>
