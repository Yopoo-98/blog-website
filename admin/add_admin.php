<?php 

include"..\includes/connection.php";

if(isset($_POST['add_admin'])){
    $first_name =htmlentities((mysqli_real_escape_string($conn,$_POST['first_name'])));
    $second_name =htmlentities((mysqli_real_escape_string($conn,$_POST['second_name'])));
    $email =htmlentities((mysqli_real_escape_string($conn,$_POST['email'])));
    $password =htmlentities((mysqli_real_escape_string($conn,$_POST['password'])));
    $confirm_password =htmlentities((mysqli_real_escape_string($conn,$_POST['confirm_password'])));
   

    $newgid = sprintf('%05d',rand(0,999999));


    $username = strtolower($first_name . "_" . $second_name . "_" . $newgid);

    $check_username_query = "SELECT user_name FROM blog_users WHERE email = '$email' ";
    $run_username = mysqli_query($conn, $check_username_query);


    $password==$confirm_password;
    if($password != $confirm_password){  
        $_SESSION['message'] = "Password Do Not Match!!!" . mysqli_error($conn);
        header("Location: admin_register.php?msg=Password Do Not Match!!!.");
        exit(0);
       
      }

    if(strlen(($password) < 6)){
        $_SESSION['message'] = "Password Must Be a Maximum of 6 Characters" . mysqli_error($conn);
        header("Location: admin_register.php?msg=Password Must Be a Maximum of 6 Characters. ");
        exit(0);
    }
    

    $check_email = "select * from admin where email = '$email' ";
    $run_email = mysqli_query($conn, $check_email);

    $check = mysqli_num_rows($run_email);
    if($check==1){
        $_SESSION['message'] = "Try another email, email  already exist !!!" . mysqli_error($conn);
        header("Location: add_admin.php?msg=Try another email, email  already exist !!!. ");
        exit(0);
    }


 
 //    $fileName = $_FILES["image"]["name"];
 //    $tmpName = $_FILES["image"]["tmp_name"];

 //    $image = 'img/'. $fileName;
   
 // if(  move_uploaded_file($tmpName,$image)){

      
        $sql ="INSERT INTO `admin`( `fname`, `lname`, `email`, `password`, `date`) 
        VALUES ('$first_name','$second_name','$email','$password',now())";
    
        $query = mysqli_query($conn, $sql);

        if($query){
            $_SESSION['message'] = "New Admin Role Added Successfully" . mysqli_error($conn);
            header("Location: ..\login.php?msg=New Admin Role Added Successfull. ");
            exit(0);
        }
        else{
            $_SESSION['message'] = "New Admin Role Added Successfull" . mysqli_error($conn);
            header("Location: admin_register.php?msg=New Admin Role Added Successfull. ");
            exit(0);
        }
    }
 
// }
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>signup</title>
     <!-- Bootstrap CSS -->
     <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet" >
     <link rel="stylesheet" href="//maxcdn.bootstrapcdn.com/font-awesome/4.3.0/css/font-awesome.min.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.1.1/css/all.min.css" integrity="sha512-KfkfwYDsLkIlwQp6LFnl8zNdLGxu9YAA1QvwINks4PhcElQSvqcyVLLD9aMhXd13uQjoXtEKNosOWaZqXgel0g==" crossorigin="anonymous" referrerpolicy="no-referrer" />
    <link rel="stylesheet" href="assets/css/bootstrap.min.css">
    <script src="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.1.1/js/all.min.js" integrity="sha512-6PM0qYu5KExuNcKt5bURAoT6KCThUmHRewN3zUFNaoI6Di7XJPTMoT6K0nsagZKk2OB4L7E3q1uQKHNHd4stIQ==" crossorigin="anonymous" referrerpolicy="no-referrer"></script>
    <script src="assets/js/bootstrap.min.js"></script>
<!-- 
     <link rel="stylesheet" href="..\signup.css"> -->

</head>
<body>

       <?php 
      // include"..\includes/header.php";
       include"..\includes/admin_sidebar.php";
      // include"function.php";
      ?><br><br>

    <div class="main-content">

        <div class="container">
            <div class="row">
                <div class="col my-5 text-center text-light">
                
                <img src="..\images/logo2.png" alt="">
                </div>
            </div>
        </div><hr style=" background:white; height:10px; margin-top:-40px">

        <br>

                <div class="container " >
                    <div class="row ">
                        <div class="col-3"></div>
                        <div class="col-6">

                                <?php
                                if(isset($_GET['msg'])){
                                $msg = $_GET['msg'];
                                echo'<div class="alert alert-warning alert-dismissible fade show" role="alert">
                                '.$msg.'
                                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                                </div>';
                                }
                                ?>

                            <form action="" method="post" autocomplete="off" enctype="multipart/form-data">
                                <h2 style="text-align: center;color:brown;font-weight:bold">AAMUSTED BLOG WEBSITE REGISTRATION<br>ADMIN</h2><br>
                                    <div class="form-group">
                                        <input type="text" class="form-control" name="first_name" placeholder="Enter First Name" style="border:2px solid brown" required>
                                    </div> 
                                    <div class="form-group"><br>
                                        <input type="text" class="form-control" name="second_name" placeholder="Enter Last Name" style="border:2px solid brown" required>
                                    </div><br>
            
                                <div class="form-group">
                                <input type="email" class="form-control" placeholder="Enter Email" name="email" style="border:2px solid brown" required>
                                </div><br>

                                    <div class="form-group">
                                        <input type="password" class="form-control" name="password" placeholder="Enter Password" style="border:2px solid brown" required>
                                    </div><br>
                                    <div class="form-group">
                                        <input type="password" class="form-control" name="confirm_password" placeholder="Confirm Password" style="border:2px solid brown" required>
                                    </div><br>
                                    <div class="form-group">
                                    

                               
                                <button class="form-control text-light fs-4 fw-bold"  name="add_admin" style="color: white;background:brown">Add New Admin</button><br>
                          
                                <div>
                                    <div class="row">
                                        <div class="col">         
                                            <p class=" text-success">Click here to <span><a href="super_admin.php" style="text-decoration:none;color:red;font-size:18px">Back</a></span></p>
                                        </div>
                                        <div class="col"></div>
                                        <div class="col">
                                             <p class=" text-success"><span><a href="super_admin_register.php" style="text-decoration:none;font-weight:bold;font-size:18px">Add New User</a></span></p>
                                        </div>
                                    </div>
                                </div>
                            </form>
                    </div>  

                    <div class="col-3"></div>

                </div>
        </div>
</div>
      <!-- Option 1: Bootstrap Bundle with Popper -->
      <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.bundle.min.js"></script>
</body>
</html>

<style>
  form{
        border: 2px solid brown;
        padding: 40px;
        border-radius: 5px;
    }
</style>